...
from ibind.ws.ws_transport import (
    TransportOpened,
    TransportClosed,
    TransportError,
    TransportMessage,
...
class TestResetWebsocketApp:
    ...
    def test_reset_raises_when_called_from_transport_thread(self, transport):
        ...
        ## Act / Assert
        with patch('ibind.ws.ws_transport.tname', return_value='test_thread'):
    ...
    def test_reset_abandons_wsa_when_close_times_out(self, transport, mock_condition):
        ...
        ## Act
        with patch('ibind.ws.ws_transport.wait_until', side_effect=[True]):
            transport.reset_websocket_app()
    ...
    def test_reset_waits_for_wsa_change_via_condition(self, transport, mock_condition):
        ...
        ## Act
        with patch('ibind.ws.ws_transport.wait_until', side_effect=[True, True]):
            result = transport.reset_websocket_app()
    ...
    def test_reset_logs_error_when_recreation_times_out(self, transport):
        ...
        ## Act
        with patch('ibind.ws.ws_transport.wait_until', return_value=False):
            result = transport.reset_websocket_app()
    ...
    def test_reset_with_running_transport_thread(self, transport, mock_get_cookie, mock_get_header):
        ...
        ## Act
        with patch('ibind.ws.ws_transport.WebSocketApp', side_effect=factory.create_mock_wsa):
...
class TestCheckPing:
    ...
    def test_check_ping_returns_true_when_within_interval(self, transport):
        ...
        ## Act
        with mock_module_time('ibind.ws.ws_transport', time_sequence=[current_time]):
            result = transport.check_ping()
...
class TestNewWsa:
    ...
    def test_new_wsa_creates_websocket_app(self, transport, mock_get_cookie, mock_get_header):
        ...
        ## Act
        with patch('ibind.ws.ws_transport.WebSocketApp') as mock_wsa_class:
            transport._new_wsa()
