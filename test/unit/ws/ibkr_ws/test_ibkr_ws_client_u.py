...
from ibind.ibkr_ws.ibkr_events import System
from ibind.ibkr_ws.ibkr_router import IbkrRouter
from ibind.ibkr_ws.ibkr_ws_client import IbkrWsClient, _build_ws_url
from ibind.ibkr_ws.ibkr_subscriptions import MarketHistorySubscription
from ibind.ws.ws_sinks import AsyncSink
from ibind.ws.ws_subscriptions import SubscriptionHandle, BindingStatus
from ibind.ws.ws_runtime import WsState
from test.test_utils import capture_logs
from ibind.ibkr_ws.ibkr_subscriptions import MarketDataSubscription


@pytest.fixture
def client():
    with patch('ibind.ibkr_ws.ibkr_ws_client.IbkrClient'), patch('ibind.ibkr_ws.ibkr_ws_client.WsRuntime'):
        return IbkrWsClient()
...
class TestBuildWsUrl:
    ...
    @patch('ibind.ibkr_ws.ibkr_ws_client.var')
...
class TestIbkrWsClientInit:
    @capture_logs()
    @patch('ibind.ibkr_ws.ibkr_ws_client.IbkrClient')
    @patch('ibind.ibkr_ws.ibkr_ws_client.WsRuntime')
    def test_default_initialization(self, mock_ws_runtime, mock_ibkr_client):
        """IbkrWsClient initializes with default parameters."""
        ## Arrange
        mock_client_instance = MagicMock()
        mock_ibkr_client.return_value = mock_client_instance

        ## Act
        with patch.object(IbkrWsClient, '_register_internal_callbacks'):
            client = IbkrWsClient()
    ...
    def test_custom_account_id(self, mock_ws_runtime, mock_ibkr_client):
        """IbkrWsClient initializes with custom account_id."""
        ## Arrange
        mock_client_instance = MagicMock()
        mock_ibkr_client.return_value = mock_client_instance
        ...
        ## Act
        with patch.object(IbkrWsClient, '_register_internal_callbacks'):
            client = IbkrWsClient(account_id='CUSTOM_ACCOUNT')
    ...
    def test_custom_ibkr_client(self, mock_ws_runtime, mock_ibkr_client):
        """IbkrWsClient uses provided ibkr_client instead of creating new one."""
        ## Arrange
        custom_client = MagicMock(spec=IbkrClient)

        ## Act
        with patch.object(IbkrWsClient, '_register_internal_callbacks'):
            client = IbkrWsClient(ibkr_client=custom_client)
    ...
    def test_noop_sink_by_default(self, mock_ws_runtime, mock_ibkr_client):
        """IbkrWsClient uses NoopSink when sink is None."""
        ## Arrange
        mock_client_instance = MagicMock()
        mock_ibkr_client.return_value = mock_client_instance
    ...
    def test_synchronous_output_events(self, mock_ws_runtime, mock_ibkr_client):
        """IbkrWsClient uses synchronous sink when synchronous_output_events is True."""
        ## Arrange
        mock_client_instance = MagicMock()
        mock_ibkr_client.return_value = mock_client_instance
        ...
        ## Act
        IbkrWsClient(sink=custom_sink, synchronous_output_events=True)
    ...
    def test_custom_router(self, mock_ws_runtime, mock_ibkr_client):
        """IbkrWsClient uses provided router."""
        ## Arrange
        mock_client_instance = MagicMock()
        mock_ibkr_client.return_value = mock_client_instance
        ...
        ## Act
        with patch.object(IbkrWsClient, '_register_internal_callbacks'):
            IbkrWsClient(router=custom_router)
    ...
    def test_custom_subscription_resolver(self, mock_ws_runtime, mock_ibkr_client):
        """IbkrWsClient uses provided subscription_resolver."""
        ## Arrange
        mock_client_instance = MagicMock()
        mock_ibkr_client.return_value = mock_client_instance
        ...
        ## Act
        with patch.object(IbkrWsClient, '_register_internal_callbacks'):
            IbkrWsClient(subscription_resolver=custom_resolver)
    ...
    def test_register_internal_callbacks_called(self, mock_ws_runtime, mock_ibkr_client):
        """IbkrWsClient registers internal callbacks during initialization."""
        ## Arrange
        mock_client_instance = MagicMock()
        mock_ibkr_client.return_value = mock_client_instance
...
class TestIbkrWsClientCallbacks:
    @capture_logs()
...
class TestIbkrWsClientGetCookie:
    @capture_logs()
...
class TestIbkrWsClientGetHeader:
    @capture_logs()
...
class TestIbkrWsClientGetAuthenticated:
    @capture_logs()
...
class TestIbkrWsClientSubscribe:
    @capture_logs()
...
class TestIbkrWsClientUnsubscribe:
    @capture_logs()
...
class TestIbkrWsClientGetStatus:
    @capture_logs()
...
class TestIbkrWsClientTic:
    @capture_logs()
    @patch('ibind.ibkr_ws.ibkr_ws_client.wait_until')
...
class TestIbkrWsClientMarketHistoryServerIdTracking:
    @capture_logs()
