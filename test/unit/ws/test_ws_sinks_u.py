...
from ibind import CallbackSink, WsState, QueueSink, CompositeSink, EventSink
from ibind.ws._ws_events import WsOpen, WsClose, WsAuthenticated
from ibind.ws.ws_sinks import AsyncSink, NoopSink
...
class TestAsyncSink:
    ...
    @capture_logs(
        logger='ibind.ibkr_ws_client',
        logger_level='DEBUG',
        error_level='ERROR',
        expected_errors=['AsyncSink thread failed to stop within timeout', 'Event queue not empty when stopping; 1 events will remain unprocessed'],
        partial_match=True,
    )
