...
class TestExecuteInParallelU(unittest.TestCase):
    ...
    def test_execute_in_parallel_rate_limiting(self):
        ...
        results = execute_in_parallel(
            slow_func, requests, max_per_second=max_per_second
        )

        duration = time.time() - start_time
        self.assertGreaterEqual(
            duration, 1.05
        )  # Should take at least 1.1 seconds to complete all requests
...
class TestWaitUntilU(unittest.TestCase):
    ...
    def test_wait_until_timeout_message(self, mock_logger_error):
        ...
        self.assertFalse(
            wait_until(condition, timeout_message=timeout_message, timeout=0.1)
        )
