...
class TestIbkrClientE(TestCase):
    def setUp(self):
        warnings.filterwarnings(
            "ignore",
            message="Unverified HTTPS request is being made to host 'localhost'",
        )
    ...
    def test_get_stocks(self):
        result = self.client.security_stocks_by_symbol(
            self.query, default_filtering=False
        )
    ...
    def test_historical_marketdata_beta(self):
        self.client.receive_brokerage_accounts()
        result = self.client.historical_marketdata_beta(
            265598, "1d", "1hrs", outside_rth=True, bar_type="Ask"
        )
        print(result)
