...
class TestIbkrUtilsI(TestCase):
    ...
    def test_filter_stocks(self):
        queries = [
            StockQuery(
                symbol="AAPL", contract_conditions={"isUS": False}, name_match="APPLE"
            ),
        ...
        with self.assertLogs(project_logger(), level="INFO") as cm:
            rv = filter_stocks(
                queries, Result(data=self.instruments), default_filtering=False
            )
        ...
        verify_log(
            ...
            cm,
            [
                f"Error getting stocks. Could not find valid instruments INVALID_SYMBOL in result: {self.result}. Skipping query={queries[-1]}."
            ],
        )
            ...
            [
                {
                    ...
                    "chineseName": None,
                    "contracts": [
                        {"conid": 42001300, "exchange": "IBIS", "isUS": False}
                    ],
            ...
            [
                {
                    ...
                    "contracts": [
                        {"conid": 169544810, "exchange": "NYSE", "isUS": True}
                    ],
                    "name": "HUBSPOT INC",
                }
            ],
        ...
        self.assertEqual(
            [
                ...
                },
                {
                    ...
                    "contracts": [
                        {"conid": 80993135, "exchange": "LSE", "isUS": False}
                    ],
            ...
            [
                {
                    ...
                    "chineseName": None,
                    "contracts": [
                        {"conid": 589316251, "exchange": "NASDAQ", "isUS": True}
                    ],
...
class TestHandleQuestionsI(TestCase):
    def setUp(self):
        self.original_result = Result(
            data=[
                {
                    "id": "12345",
                    "message": ["price exceeds the Percentage constraint of 3%."],
                }
            ],
    ...
    def test_successful_handling(self, question_type_mock):
        ...
        question_type_mock.PRICE_PERCENTAGE_CONSTRAINT.__str__.return_value = (
            "price exceeds the Percentage constraint of 3%."
        )
        question_type_mock.ADDITIONAL_QUESTION_TYPE.__str__.return_value = (
        ...
        replies = [
            Result(
                data=[{"id": "12346", "message": ["This is an additional question."]}],
                request={"url": "another_question_url"},
            ),
            Result(
                data=[{"id": "12347"}], request={"url": "final_url"}
            ),  # No more questions
        ]
        self.reply_callback.side_effect = replies

        result = handle_questions(
            self.original_result, self.answers, self.reply_callback
        )
        ...
        expected_calls = [
            call(
                self.original_result.data[0]["id"],
                self.answers[question_type_mock.PRICE_PERCENTAGE_CONSTRAINT],
            ),  # First call with question ID '12346' and reply True
            call(
                replies[0].data[0]["id"],
                self.answers[question_type_mock.ADDITIONAL_QUESTION_TYPE],
    ...
    def test_multiple_orders_returned(self):
        ...
        verify_log(
            ...
            cm,
            [
                "While handling questions multiple orders were returned: "
                + pformat(self.original_result.data)
            ],
        )
    ...
    def test_multiple_messages_returned(self):
        ...
        self.original_result.data = [
            {
                "id": "12345",
                "message": [str(QuestionType.PRICE_PERCENTAGE_CONSTRAINT), "Message 2"],
            }
        ]
        self.reply_callback.return_value = self.original_result.copy(data=[{}])
        ...
        verify_log(
            ...
            cm,
            [
                "While handling questions multiple messages were returned: "
                + pformat(self.original_result.data[0]["message"])
            ],
        )
