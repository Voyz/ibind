...
from ibind.client.ibkr_ws_client import (
    IbkrWsClient,
...
class TestPreprocessRawMessage(TestCase):
    ...
    def test_preprocess_with_well_formed_message(self):
        ...
        self.assertEqual(
            self.ws_client._preprocess_raw_message(raw_message), expected_result
        )
...
class TestIbkrWsClient(TestCase):
    ...
    def setUp(self):
        ...
        self.thread_mock.start.side_effect = lambda: self.ws_client._run_websocket(
            self.wsa_mock
        )
    ...
    def run_in_test_context(
        self, fn, expected_errors: list[str] = None, expect_logs: bool = True
    ):
        with patch(
            "ibind.base.ws_client.WebSocketApp",
            side_effect=lambda *args, **kwargs: init_wsa_mock(
                self.wsa_mock, *args, **kwargs
            ),
        ), patch(
            "ibind.base.ws_client.Thread", return_value=self.thread_mock
        ) as new_thread_mock, SafeAssertLogs(
            self, "ibind", level="DEBUG", logger_level="DEBUG", no_logs=not expect_logs
        ) as cm, RaiseLogsContext(
            self, "ibind", level="WARNING", expected_errors=expected_errors
        ):
    ...
    def _send_payload(
        self, payload: dict, expected_errors: list[str] = None, expect_logs: bool = True
    ):
        def run():
        ...
        return self.run_in_test_context(
            run, expected_errors=expected_errors, expect_logs=expect_logs
        )
    ...
    def test_on_message_system_heartbeat(self):
        hb = 12345678
        cm, success = self._send_payload(
            {"topic": "system", "hb": hb}, expect_logs=False
        )
    ...
    def test_on_message_blt(self):
        ...
        with patch.object(
            self.ws_client, "_handle_bulletin", MagicMock()
        ) as mock_handle_bulletin:
    ...
    def test_on_message_sts_unauthenticated(self):
        ...
        with patch("ibind.base.rest_client.requests") as requests_mock:
            ...
            cm, success = self._send_payload(
                message_data, expected_errors=expected_errors
            )
    ...
    def test_on_message_no_topic_handler(self):
        message_data = {
            "topic": "unrecognized_topic",
            "args": {"some_key": "some_value"},
        }
        expected_errors = [
            f'IbkrWsClient: Topic "{message_data["topic"]}" unrecognised. Message: {message_data}'
        ]
    ...
    def test_on_message_handled_without_subscription(self):
        ...
        with patch.object(
            self.ws_client, "_handle_subscribed_message", return_value=True
        ):
            cm, success = self._send_payload(
                message_data, expected_errors=expected_errors
            )
    ...
    def _logs_subscriptions(
        self,
        full_channel,
        data=None,
        needs_confirmation_sub: bool = False,
        needs_confirmation_unsub: bool = True,
    ):
        return [
    ...
    def test_on_message_market_data_channel_handling(self):
        ...
        self.assertEqual(
            self._logs_subscriptions(full_channel, request["data"]),
    ...
    def test_on_message_trade_channel_handling(self):
        ...
        self.assertEqual(
            self._logs_subscriptions(full_channel), [r.msg for r in cm.records]
        )
        self.assertEqual(response, queue.get())
    ...
    def test_on_message_orders_channel_handling(self):
        ...
        self.assertEqual(
            self._logs_subscriptions(full_channel, None, True, True),
            [r.msg for r in cm.records],
        )
        self.assertEqual(response, queue.get())
    ...
    def test_subscription_without_confirmation(self):
        ...
        with patch.object(self.ws_client, "has_subscription", return_value=True):
            cm, success = self._subscribe(
                request, response, expected_errors=expected_errors
            )
            self.assertTrue(success)
    ...
    def test_check_health(self):
        ...
        def run():
            ...
            def override_init_wsa_mock(wsa_mock: MagicMock, *args, **kwargs):
                ...
                wsa_mock._on_message.side_effect = (
                    lambda wsa_mock, message: wsa_mock.__on_message__(
                        wsa_mock, json.dumps(response)
                    )
                )
                return wsa_mock
            ...
            self.wsa_mock._on_message.side_effect = (
                lambda wsa_mock, message: wsa_mock.__on_message__(
                    wsa_mock, json.dumps(response)
                )
            )
            ...
            with patch("ibind.client.ibkr_ws_client.time") as time_mock, patch.object(
                self.ws_client, "check_ping", return_value=True
            ), patch(
                "ibind.base.ws_client.WebSocketApp",
                side_effect=lambda *args, **kwargs: override_init_wsa_mock(
                    self.wsa_mock, *args, **kwargs
                ),
            ), patch.object(
                self.ws_client,
                "_has_active_connection",
                side_effect=has_active_connection,
            ) as has_active_connection_mock:
                ...
                self.assertEqual(
                    [call()] * 6, has_active_connection_mock.call_args_list
                )
                self.ws_client.shutdown()
        ...
        channel_subscribed_log = (
            f"IbkrWsClient: Subscribed: s{full_channel}+{json.dumps(request['data'])}"
        )
