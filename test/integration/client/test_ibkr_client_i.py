...
class TestIbkrClientI(TestCase):
    ...
    def test_get_conids(self, requests_mock):
        ...
        queries = [
            StockQuery(
                symbol="AAPL",
                contract_conditions={"isUS": False, "exchange": "AEQLIT"},
                name_match="APPLE",
            ),
            ...
            StockQuery(
                symbol="GOOG",
                contract_conditions={"isUS": False, "exchange": "MEXI"},
            ...
            StockQuery(
                symbol="SAN", name_match="SANTANDER", contract_conditions={"isUS": True}
            ),
    ...
    def test_get_conids_exception(self, requests_mock):
        ...
        symbol = "AAPL"
        query = StockQuery(
            symbol=symbol, contract_conditions={"isUS": False}, name_match="APPLE"
        )

        instruments = filter_stocks(
            query,
            Result(data={symbol: ibkr_responses.responses["stocks"][symbol]}),
    ...
    def test_get_live_orders_with_valid_filters(self, requests_mock):
        ...
        self.client.get.assert_called_with(
            "iserver/account/orders", params={"filters": "inactive,filled"}
        )
    ...
    def test_get_live_orders_with_single_filter(self, requests_mock):
        ...
        self.client.get.assert_called_with(
            "iserver/account/orders", params={"filters": "submitted"}
        )
    ...
    def _marketdata_request(self, method, url, *args, **kwargs):
        ...
        if leaf == "stocks":
            return MagicMock(
                json=lambda: ibkr_responses.responses["stocks"]
            )  # Mock response for get_conids
        elif leaf == "history":
    ...
    def test_marketdata_history_by_symbols(self, requests_mock):
        ...
        self._history_by_conid = {
            ibkr_responses.responses["filtered_conids"][key]: value
            for key, value in ibkr_responses.responses["history"].items()
        }
        ...
        expected_errors = [
            "Market data for CDN is not live: Delayed",
            "Market data for CFC is not live: Delayed",
        ]

        with SafeAssertLogs(
            self, "ibind", level="INFO", logger_level="DEBUG", no_logs=False
        ) as cm, RaiseLogsContext(
            self, "ibind", level="ERROR", expected_errors=expected_errors
        ):
    ...
    def test_check_health_authenticated_and_connected(self, requests_mock):
        ...
        self.client.tickle = MagicMock(
            return_value=Result(data=response_data, request={"url": self.default_url})
        )
    ...
    def test_check_health_connection_error(self, requests_mock):
        ...
        self.assertFalse(health_status)
        self.assertIn(
            "ConnectTimeout raised when communicating with the Gateway", cm.output[0]
        )
    ...
    def test_marketdata_unsubscribe_success(self, requests_mock):
        ...
        requests_mock.request.side_effect = lambda method, url, **kwargs: responses[
            kwargs["json"]["conid"]
        ]
        self.client.get = MagicMock(
            side_effect=lambda url, *args, **kwargs: Result(
                data={"success": True}, request={"url": url}
            ),
    ...
    def test_marketdata_unsubscribe_raises_exception_on_failure(self, requests_mock):
        ...
        self.client.post = MagicMock(
            side_effect=lambda url, *args, **kwargs: ExternalBrokerError(
                status_code=500
            ),
            __name__="client_get_mock",
        )
