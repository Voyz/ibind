...
def test_get_conids(client_fixture, mocker):
    ...
    expected_error_substring = "Could not find valid instruments INVALID_SYMBOL"
    # Act
    with CaptureLogsContext('ibind.ibkr_utils', level='ERROR', expected_errors=[expected_error_substring], partial_match=True) as cm:
        rv = client.stock_conid_by_symbol(queries, default_filtering=False)

    # Assert
    assert expected_error_substring in cm.output[0]
...
def test_marketdata_history_by_symbols(client_fixture, mocker):
    ...
    for symbol, expected in expected_results.items():
        ...
        assert result['close'] == expected['close']
...
def test_check_health_connection_error(client_fixture, mocker):
    ...
    # Act
    with CaptureLogsContext('ibind.session_mixin', level='ERROR', expected_errors=['ConnectTimeout raised when communicating with the Gateway. This could indicate that the Gateway is not running or other connectivity issues.']) as cm:
        health_status = client.check_health()
...
def test_marketdata_unsubscribe_failure_returns_exception(client_fixture, mocker):
    # Arrange
    client, _, _, _, _, _, _ = client_fixture
    conids = [12345]
    ...
    # Assert
    assert isinstance(results['12345'], ExternalBrokerError)
    assert results['12345'].status_code == 500
