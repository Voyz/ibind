...
class TestWsClient(TestCase):
    ...
    def run_in_test_context(self, fn, expected_errors: list[str] = None):
        ...
        ) as new_thread_mock, self.assertLogs(
            "ibind", level="DEBUG"
    ...
    def start(self):
        ...
        self.new_thread_mock.assert_called_with(
            target=self.ws_client._run_websocket,
            args=(self.wsa_mock,),
    ...
    def _logs_failed_attempt(self, attempt):
        ...
        if attempt:
            s.append(
                f"WsClient: Connect reattempt {attempt}/{self.max_reconnect_attempts}"
            )
        return s
    ...
    def _verify_started(self):
        self.wsa_mock.run_forever.assert_called_with(
            sslopt=self.ws_client._sslopt,
            ping_interval=self.ws_client._ping_interval,
            ping_timeout=0.95 * self.ws_client._ping_interval,
        )
    ...
    def test_start_success_on_second_attempt(self):
        ...
        cm, success = self.run_in_test_context(
            self.start, expected_errors=expected_errors
        )
    ...
    def test_start_reattempt_failure(self):
        ...
        expected_logs.append(
            f"WsClient: Connection failed after {self.max_reconnect_attempts} attempts"
        )
    ...
    def test_open_exception(self):
        ...
        self.wsa_mock.run_forever.side_effect = (
            lambda *args, **kwargs: run_forever_exception(
                self.wsa_mock, *args, **kwargs
            )
        )
