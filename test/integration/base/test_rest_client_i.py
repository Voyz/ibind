...
class TestRestClientI(TestCase):
    ...
    def test_default_rest(self, requests_mock):
        ...
        requests_mock.request.assert_called_with(
            "GET", self.default_url, verify=False, headers={}, timeout=self.timeout
        )
        ...
        self.assertEqual(self.result, rv)
        requests_mock.request.assert_called_with(
            "DELETE", self.default_url, verify=False, headers={}, timeout=self.timeout
        )

    def test_request_retries(self, requests_mock):
        requests_mock.request.side_effect = ReadTimeout()

        with self.assertLogs(project_logger(), level="INFO") as cm, self.assertRaises(
            TimeoutError
        ) as cm_err:
        ...
        for i, record in enumerate(cm.records):
            self.assertEqual(
                f"RestClient: Timeout for GET {self.default_url} {{}}, retrying attempt {i + 1}/{self.max_retries}",
                record.msg,
            )
        self.assertEqual(
            f"RestClient: Reached max retries ({self.max_retries}) for GET {self.default_url} {{}}",
    ...
    def test_response_raise_timeout(self, requests_mock):
        ...
        self.assertEqual(
            f"RestClient: Timeout error ({self.timeout}S)", str(cm_err.exception)
        )
