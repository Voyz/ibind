...
from ibind.ws.ws_subscriptions import BindingStatus
from ibind.ws.ws_transport import TransportOpened, TransportClosed, TransportError, TransportMessage
...
class TestRestartAndHardResetScenarios:
    ...
    def test_hard_reset_reenables_transport_callbacks(self, runtime):
        ...
        with (
            ...
            patch('ibind.ws.runtime.ws_lifecycle.wait_until', return_value=True),
        ):
            ## Act
            runtime.hard_reset()
    ...
    @capture_logs(
        logger_level='ERROR',
        expected_errors=[
            'Failed to stop transport thread',
            'Transport thread failed to stop within timeout',
            'Runtime thread failed to stop within timeout',
            'Stop incomplete',
        ],
        partial_match=True,
    )
