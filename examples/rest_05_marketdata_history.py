...
st = time.time()
history = client.marketdata_history_by_conid('265598', period='1min', bar='1min', outside_rth=True)
diff_one_conid = time.time() - st
print('#### One conid ####')
print(f'{history}')

st = time.time()
history = client.marketdata_history_by_symbol('AAPL', period='1min', bar='1min', outside_rth=True)
diff_one_symbol_raw = time.time() - st
print('\n\n#### One symbol raw ####')
print(f'{history}')

st = time.time()
history = client.marketdata_history_by_symbols('AAPL', period='1min', bar='1min', outside_rth=True)
diff_one_symbol = time.time() - st
print('\n\n#### One symbol ####')
print(f'{history}')

st = time.time()
history = client.marketdata_history_by_symbols(['AAPL', 'MSFT', 'GOOG', 'TSLA', 'AMZN'], period='1min', bar='1min', outside_rth=True)
diff_five_symbols = time.time() - st
print('\n\n#### Five symbols ####')
print(f'{history}')

st = time.time()
history = client.marketdata_history_by_symbols(['AAPL', 'MSFT', 'GOOG', 'TSLA', 'AMZN', 'ADBE', 'AMD', 'COIN', 'META', 'DIS', 'BAC', 'XOM', 'KO', 'WMT', 'V'], period='1min', bar='1min', outside_rth=True)
