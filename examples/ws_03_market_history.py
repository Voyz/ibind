...
def unsubscribe():
    ...
    for server_id, conid in ws_client.server_ids(IbkrWsKey.MARKET_HISTORY).items():
        ...
        if (
            conid is not None
        ):  # if we know the conid let's try to confirm the unsubscription
            channel += f"+{conid}"
            needs_confirmation = True

        confirmed = ws_client.unsubscribe(
            channel, server_id, needs_confirmation, subscription_processor
        )

        print(
            f"Unsubscribing channel {channel!r} from server {server_id!r}: {'unconfirmed' if not confirmed else 'confirmed'}."
        )
