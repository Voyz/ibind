...
from support.py_utils import ensure_list_arg, execute_in_parallel

_LOGGER = project_logger(__file__)


class IbkrClient(RestClient):
    """
    ...
    def auth_status(self) -> Result:  # pragma: no cover
        return self.post('iserver/auth/status')

    def reauthenticate(self) -> Result:  # pragma: no cover
        return self.post('iserver/reauthenticate')

    def log_out(self) -> Result:  # pragma: no cover
        return self.post('logout')

    def get_accounts(self) -> Result:  # pragma: no cover
        return self.get('portfolio/accounts')

    def get_ledger(self) -> Result:  # pragma: no cover
        return self.get(f'portfolio/{self.account_id}/ledger')

    def get_positions(self, page: int = 0) -> Result:  # pragma: no cover
        return self.get(f'portfolio/{self.account_id}/positions/{page}')

    def get_order(self, order_id: str) -> Result:  # pragma: no cover
    ...
    def marketdata_history_by_symbols(self, symbols: List[str]) -> dict:
        conids = self.get_conids(symbols).data

        requests = {symbol: {"kwargs":
                                 {'conid': conid, "period": "1min", "bar": "1min", "outside_rth": True}
    ...
    def reply(self, question_id: str, reply: bool) -> Result:  # pragma: no cover
        return self.post(f'iserver/reply/{question_id}', params={"confirmed": reply})

    def portfolio_invalidate(self, account_id: str = None) -> Result:  # pragma: no cover
        if account_id is None:
            account_id = self.account_id
        return self.post(f'portfolio/{account_id}/positions/invalidate')

    def marketdata_unsubscribe_all(self) -> Result:  # pragma: no cover
        return self.get(f'iserver/marketdata/unsubscribeall')

    def cancel_order(self, order_id: str, account_id: str = None) -> Result:  # pragma: no cover
        if account_id is None:
            account_id = self.account_id
    ...
    def marketdata_unsubscribe(self, conids: [int]):
        ...
        results = execute_in_parallel(self.get, unsubscribe_requests)
    ...
    def get_live_orders(self, filters: Union[list, str] = None, force:bool=False) -> Result:
        """
    ...
    def get_stocks(self, queries: [StockQuery], default_filtering: bool = True) -> Result:
        """
    ...
    @ensure_list_arg('queries')
    def get_conids(self, queries: [StockQuery], default_filtering: bool = True, return_type='dict') -> Result:
        """
    ...
    def submit_order(self, order_request: dict, answers: Answers, account_id: str = None) -> Result:
        """
        Keep this in mind:
