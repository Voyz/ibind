...
class OAuthMixin():  # pragma: no cover
    ...
    def req_live_session_token(self) -> tuple[str, int]:
        ...
        dh_prime=self.pem_to_dh_prime(pem_file_path=config['ibkr']['DH_PRIME_FP'])

        dh_random = self.generate_dh_random_bytes()
        dh_challenge = self.generate_dh_challenge(
            dh_prime=dh_prime,
            dh_generator=int(config['ibkr']['DH_GENERATOR']),
            dh_random=dh_random,
        )
        prepend = self.calculate_live_session_token_prepend(
        ...
        live_session_token = self.calculate_live_session_token(
            dh_prime=dh_prime,
    ...
    def get_headers(
        ...
        headers = {
            ...
            "oauth_nonce": self.generate_oauth_nonce(),
            "oauth_signature_method": signature_method,
            "oauth_timestamp": self.generate_request_timestamp(),
        }
        if oauth_token:
        ...
        if signature_method == "HMAC-SHA256":
            headers.update(
                {
                    "oauth_signature": self.generate_hmac_sha_256_signature(
            ...
            headers.update(
                {
                    "oauth_signature": self.generate_rsa_sha_256_signature(
                        base_string=base_string,
                        private_signature_key=self.read_private_key(
        ...
        header_oauth={
                "Authorization": self.generate_authorization_header_string(
    ...
    def generate_request_timestamp(self) -> str:
        """
    ...
    def read_private_key(self,private_key_fp: str) -> RSA.RsaKey:
        """
    ...
    def generate_oauth_nonce(self) -> str:
        """
    ...
    def generate_dh_random_bytes(self) -> str:
        """
    ...
    def generate_dh_challenge(self,dh_prime: str, dh_random: str, dh_generator: int = 2) -> str:
        """
    ...
    def generate_hmac_sha_256_signature(self,base_string: str, live_session_token: str) -> str:
        """
    ...
    def get_access_token_secret_bytes(self,access_token_secret: str) -> list[int]:
        """
    ...
    def to_byte_array(self,x: int) -> list[int]:
        """
    ...
    def calculate_live_session_token(
        ...
        INT_BASE = 16
        STRING_ENCODING = "utf-8"
        access_token_secret_bytes = self.get_access_token_secret_bytes(prepend)
        a = int(dh_random_value, INT_BASE)
        B = int(dh_response, INT_BASE)
        K = pow(B, a, int(dh_prime, INT_BASE))
        hmac = HMAC.new(bytes(self.to_byte_array(K)), digestmod=SHA1)
    ...
    def generate_authorization_header_string(self,request_data: dict, realm: str) -> str:
        """
