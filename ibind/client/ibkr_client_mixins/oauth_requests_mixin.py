...
from ibind.client.ibkr_client_mixins.oauth_utils_mixin import OAuth_Utils_Mixin



# Helper functions

class OAuth_Requests_Mixin(OAuth_Utils_Mixin):
    def setup_logger():
    ...
    def live_session_token(access_token: str,access_token_secret: str) -> tuple[str, int]:
        ...
        dh_prime=pem_to_dh_prime(pem_file_path=dh_prime_fp)
    ...
    def contract_information_by_conid( conid: str, access_token:str, live_session_token:str): 
            """
    ...
    def portfolio_account_summary(access_token: str, live_session_token: str,account_id:str) -> requests.Response:
        return send_oauth_request(
            request_method="GET",
    ...
    def place_order(
        ...
        answers    
            ...
            return handle_questions(result, answers, reply)


    def handle_questions(original_result, answers, reply_callback: callable) :
        """
    ...
    def pass_result(data: dict, old_result) :
        return old_result.copy(data=data)

    def find_answer(question: str, answers):
        """
