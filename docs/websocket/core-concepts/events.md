...
All lifecycle events (`WsStarting`, `WsStopping`, `WsStopped`, `WsOpen`, `WsAuthenticated`, `WsDegraded`, `WsReady`, `WsClose`, `WsError`) include `previous_state` and `current_state` attributes indicating the state transition.
